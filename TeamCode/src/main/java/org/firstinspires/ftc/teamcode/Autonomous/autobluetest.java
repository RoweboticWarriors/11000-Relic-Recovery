...
@Autonomous(name="Autodrive Blue test", group="Pushbot")
//@Disabled
public class autobluetest extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        */
        ...
        //move 180 left neg, right pos, time longer

        // Movement Based on jewel because values change based on jewel color
        if (blue = true) {
            encoderDrive(TURN_SPEED, -1.3, -1.3, 1.1);
        } else {
            encoderDrive(TURN_SPEED, -1.2, -1.2, 1.1); //move back from plate
        }

        encoderDrive(TURN_SPEED, .5, -.5, .6);
        encoderDrive(TURN_SPEED, -1, -1, 1.05);
        ...
        encoderDrive(TURN_SPEED,  1,  1, .8);

        armdrive(1.9, -1, 1);

        /* Need to make it to where the arm moves down so we can place the glyph accurately */

        // maybe close hands just enough to drop glyph

        claws(.3, .2, .6);                 //open hands
        encoderDrive(.125,  .23,  .23, .85);   //pushes block
